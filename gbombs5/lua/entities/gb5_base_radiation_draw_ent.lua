...
function ENT:Think()
	if (CLIENT) then
		function Radiation()
			...
			surface.SetDrawColor( 255, 255, 255, LocalPlayer():GetNWInt("rad_relation", 0)*255 );		
	...
	for k, v in pairs(ents.FindInSphere(pos,self.RadRadius)) do
		if v:IsPlayer() and !v:IsNPC() and v.hazsuited==false then
			...
			v:SetNWInt("rad_relation", relation  )
		...
		for k, v in pairs(self.Plylist) do
			if v:IsValid() then
				if v:GetNWInt("Affected", 0  ) then
					v:SetNWInt("Affected", 0  )
					v:SetNWInt("rad_relation", 0  )
				end
			end
		end
	end
