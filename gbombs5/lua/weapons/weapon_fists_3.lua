...
function SWEP:DealDamage()
	...
	if ( SERVER && IsValid( tr.Entity ) && ( tr.Entity:IsNPC() || tr.Entity:IsPlayer() || tr.Entity:Health() > 0 ) ) then
		...
		dmginfo:SetDamage( math.random( 100, 200 ) )
	...
	if ( SERVER && IsValid( tr.Entity ) ) then
		...
		if ( IsValid( phys ) ) then
			...
			phys:ApplyForceOffset( self.Owner:GetAimVector() * 1580 * phys:GetMass(), tr.HitPos )
		end
	end
	...
	self.Owner:ConCommand(" say Shadow Dragon's Slash!\n")
...
function SWEP:Deploy()
	...
	ParticleEffectAttach("shadow_cast_endless",PATTACH_POINT_FOLLOW,self.Owner,self.Owner:LookupAttachment("anim_attachment_RH"))
	ParticleEffectAttach("shadow_cast_endless",PATTACH_POINT_FOLLOW,self.Owner,self.Owner:LookupAttachment("anim_attachment_LH"))
	self:UpdateNextIdle()
